...
const RetailerDashboard = ({ token, user }) => {
  ...
  useEffect(() => {
    ...
    // Join this retailer's personal room
    socket.emit('join', user.id);

    // Order placed by this retailer — add to their list immediately
    const onOrderPlaced = (order) => {
      setOrders(prev => {
        if (prev.find(o => o._id === order._id)) return prev;
        return [order, ...prev];
      });
      // Also reduce product stock in the product list live
    ...
    // Payment confirmed — update isPaid on order
    const onPaymentDone = ({ orderId, isPaid }) => {
      setOrders(prev => prev.map(o => o._id === orderId ? { ...o, isPaid } : o));
    };

    // Product stock updated by someone else ordering
  ...
  const placeOrder = async ({ productId, quantity, pricePerUnit }) => {
    ...
    try {
      ...
      // Socket 'order_placed' event will update the state — no need to call load()
  ...
  const confirmPay = async (orderId) => {
    ...
    try {
      ...
      setQrOrder(null);
      // Add new payment to payments list live
      if (data.payment) {
        setPayments(prev => [data.payment, ...prev]);
      }
      // Socket 'payment_done' will update the order's isPaid flag
    } catch (e) {
